...
export function sanitizeGraph(input: unknown): PdfGraph {
  ...
  const repo = {
    name: text(input.repo.name, "repo.name"),
    org: text(input.repo.org, "repo.org"),
    branch: text(input.repo.branch, "repo.branch"),
  };
    ...
    if (isRecord(data.meta)) {
      for (const key of ["complexity", "loc", "coverage", "churn", "contributors", "columns", "relations", "route", "method"]) {
        if (data.meta[key] !== undefined) {
          meta[key] = typeof data.meta[key] === "number" ? finite(data.meta[key], `node.meta.${key}`) : text(data.meta[key], `node.meta.${key}`, 80);
        }
      }
    }
    return {
      id: text(raw.id, `node[${index}].id`, 120),
      kind: data.kind as PdfNode["kind"],
      name: text(data.name, `node[${index}].name`, 120),
      typeLabel: text(data.typeLabel, `node[${index}].typeLabel`, 80),
      description: text(data.description, `node[${index}].description`),
      files: Array.isArray(data.files) ? data.files.slice(0, 8).map((file, fileIndex) => text(file, `node[${index}].files[${fileIndex}]`, 180)) : [],
      meta,
  ...
  const edges = input.edges.map((raw, index): PdfEdge => {
    ...
    const source = text(raw.source, `edge[${index}].source`, 120);
    const target = text(raw.target, `edge[${index}].target`, 120);
...
export async function generatePdf(graph: PdfGraph): Promise<Uint8Array> {
  ...
  pdf.setTitle(`${graph.repo.name} visual study`);
  pdf.setAuthor("Atlas");
  const pageWidth = 792;
  ...
  const maxX = Math.max(...graph.nodes.map((node) => node.position.x + 200));
  ...
  for (const edge of graph.edges) {
    ...
    const a = point(source); const b = point(target);
    overview.drawLine({ start: { x: a.x, y: a.y }, end: { x: b.x, y: b.y }, thickness: Math.max(0.35, scale), color: rgb(0.72, 0.72, 0.76), opacity: 0.65 });
  }
  for (const node of graph.nodes) {
    const p = point(node); const width = Math.max(28, 150 * scale); const height = Math.max(14, 52 * scale); const color = COLORS[node.kind];
    overview.drawRectangle({ x: p.x, y: p.y, width, height, color: rgb(0.98, 0.98, 0.99), borderColor: color, borderWidth: Math.max(0.6, 2 * scale) });
    if (scale > 0.28) overview.drawText(safePdfText(node.name).slice(0, 24), { x: p.x + 4, y: p.y + height / 2 - 3, size: Math.max(4, Math.min(9, 9 * scale + 2)), font: bold, color: rgb(0.1, 0.1, 0.12), maxWidth: width - 8 });
  }
  ...
  for (let start = 0; start < graph.nodes.length; start += 12) {
    const chunk = graph.nodes.slice(start, start + 12);
    ...
    chunk.forEach((node, index) => {
      const x = 30 + (index % 2) * 370; const y = 525 - Math.floor(index / 2) * 80; const color = COLORS[node.kind];
      page.drawRectangle({ x, y: y - 52, width: 340, height: 64, color: rgb(0.98, 0.98, 0.99), borderColor: color, borderWidth: 1.2 });
      page.drawText(`${safePdfText(node.typeLabel)} - ${safePdfText(node.name)}`.slice(0, 70), { x: x + 10, y: y - 5, size: 10, font: bold, color: rgb(0.1, 0.1, 0.12), maxWidth: 320 });
      const meta = Object.entries(node.meta).slice(0, 4).map(([key, value]) => `${key}: ${value}`).join(" | ");
      page.drawText(wrap(`${node.description}${meta ? ` | ${meta}` : ""}`, 65).slice(0, 2).join("\n"), { x: x + 10, y: y - 20, size: 7.5, lineHeight: 10, font, color: rgb(0.3, 0.3, 0.34), maxWidth: 320 });
    });
    const relationships = graph.edges.filter((edge) => chunk.some((node) => node.id === edge.source || node.id === edge.target)).slice(0, 12);
    page.drawText("Relationships in this section", { x: 30, y: 72, size: 10, font: bold, color: rgb(0.1, 0.1, 0.12) });
    relationships.forEach((edge, index) => {
      const source = byId.get(edge.source)?.name ?? edge.source; const target = byId.get(edge.target)?.name ?? edge.target;
      page.drawText(`${safePdfText(source)} -> ${safePdfText(target)} | ${edge.kind}`, { x: 30, y: 56 - index * 12, size: 7.5, font, color: rgb(0.3, 0.3, 0.34), maxWidth: 730 });
    });
  }
  return pdf.save();
}